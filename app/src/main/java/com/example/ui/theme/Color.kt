package com.example.ui.theme

import androidx.compose.ui.graphics.Color

// Light Theme (Clean Minimalism - Google Blue Philosophy)
val LightPrimary = Color(0xFF1A73E8) // Standard Premium Google Blue
val LightOnPrimary = Color(0xFFFFFFFF)
val LightBackground = Color(0xFFFFFFFF) // Crisp white background
val LightOnBackground = Color(0xFF1F1F1F) // Dark charcoal text
val LightSurface = Color(0xFFFFFFFF) // Crisp white surface
val LightOnSurface = Color(0xFF1F1F1F)
val LightOutline = Color(0xFFE0E0E0) // Soft premium light grey border
val LightSurfaceVariant = Color(0xFFF1F3F4) // Google light gray for elements
val LightOnSurfaceVariant = Color(0xFF5F6368) // Standard gray body text

// Dark Theme (Clean Minimalism - Dark Slate)
val DarkPrimary = Color(0xFF8AB4F8) // Soft light blue accent for dark theme
val DarkOnPrimary = Color(0xFF1A53AA)
val DarkBackground = Color(0xFF121212) // Pure premium dark mode background
val DarkOnBackground = Color(0xFFE3E3E3) // Crisp light gray text
val DarkSurface = Color(0xFF1E1E1E) // Slate dark surface cards
val DarkOnSurface = Color(0xFFE3E3E3)
val DarkOutline = Color(0xFF444746) // Subtle border color for dark mode
val DarkSurfaceVariant = Color(0xFF2D2E30) // Elevated card background
val DarkOnSurfaceVariant = Color(0xFFC4C7C5) // Subtitle gray for dark mode

// Premium UI Slate Palette (No Greens!)
val NabihBlue = Color(0xFF1A73E8)
val NabihBlueLight = Color(0xFFE8F0FE)
val NabihSlate = Color(0xFF5F6368)
val NabihRed = Color(0xFFD93025) // Standard Google Red
val NabihDarkSlate = Color(0xFF202124)
