package com.example.settings.profile
    ...
    suspend fun validateAndSaveApiKeys(
        ...
        if (trimmedGoogle.isNotEmpty() && trimmedGoogle != settings.value.googleApiKey) {
            val res = validateKeyGeneric("Google Gemini", trimmedGoogle, isArabic) {
                com.example.models.NetworkClient.geminiService.generateContent(
                    model = com.example.models.getCurrentGeminiModelName(),
                    apiKey = trimmedGoogle,
                    request = com.example.models.GeminiRequest(
                        contents = listOf(
                            com.example.models.GeminiContent(
                                parts = listOf(com.example.models.GeminiPart(text = "Ping"))
                            )
                        ),
