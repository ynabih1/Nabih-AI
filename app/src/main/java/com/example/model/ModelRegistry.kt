...
object ModelRegistry {
    ...
    fun getModelById(context: Context, id: String): ModelMetadata {
        ...
        if (target == null) {
            ...
            target = when (id) {
                "gemini-flash-latest", "gemini-1.0-pro", "gemini-1.5-flash", "gemini-2.0-flash", "gemini-2.5-flash", "gemini-2.0-flash-thinking-exp" -> models.find { it.id == "gemini-1.5-pro" }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (apiKey.isNotEmpty() && !apiKey.contains("YOUR_GEMINI_API_KEY") && !apiKey.contains("MY_GEMINI_API_KEY")) {
                    try {
                        val response = com.example.data.remote.NetworkClient.geminiService.listModels(apiKey)
                ...
                val updated = getModels(context).map { model ->
                    if (model.id == "nabih-ultra") {
                        ...
                        model.copy(version = "v1.5.1-live")
                    } else {
                        model
                    }
                }
