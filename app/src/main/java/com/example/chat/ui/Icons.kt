...
fun ProviderIcon(
    ...
    Box(
        ...
        when (provider) {
            ApiProvider.GOOGLE -> GeminiIcon(modifier = Modifier.matchParentSize(), color = color)
            ApiProvider.OPENAI -> OpenAiIcon(modifier = Modifier.matchParentSize(), color = color ?: com.example.ui.theme.AccentPrimary)
...
@Composable
fun GeminiIcon(modifier: Modifier = Modifier, color: Color? = null) {
    val primaryColor = color ?: com.example.ui.theme.AccentPrimary
    Canvas(modifier = modifier) {
        ...
        drawPath(
            ...
            brush = Brush.linearGradient(
                colors = listOf(primaryColor, primaryColor.copy(alpha = 0.75f), primaryColor)
            )
        )
    }
}
